...
- monitor live snapshots, review the final planform overlay, and check step-vs-sinuosity stability

## Sinuosity stability diagnostics

Each run now writes a step-vs-sinuosity history that helps identify when the planform becomes stable or quasi-stable. The saved files are:

- `Output/<id_files>/files/sinuosity_history_<id_files>.csv`
- `Output/<id_files>/plot/sinuosity_history_<id_files>.png`

The GUI also reports the current sinuosity, the relative spread over the selected stability window, the relative trend per step, and a qualitative state: not stable, quasi-stable, or stable. The default stability window is 100 stored values.
